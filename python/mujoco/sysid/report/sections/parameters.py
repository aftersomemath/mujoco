    ...
    def get_context(self) -> Dict[str, Any]:
        ...
        for param_name, param in self._opt_params.parameters.items():
            ...
            if param.size == 1:
                ...
                if param.frozen:
                    table_data.append(create_table_row(param_name, flat_value, lb, ub, is_frozen=True))
                else:
                    val = self._x_hat[non_frozen_idx]
                    table_data.append(create_table_row(param_name, val, lb, ub, idx=non_frozen_idx))
                    non_frozen_idx += 1
            else:
                for i in range(param.size):
                    ...
                    if param.frozen:
                        table_data.append(create_table_row(element_name, val_frozen, lb, ub, is_frozen=True))
                    else:
                        val = self._x_hat[non_frozen_idx]
                        table_data.append(create_table_row(element_name, val, lb, ub, idx=non_frozen_idx))
                        non_frozen_idx += 1
