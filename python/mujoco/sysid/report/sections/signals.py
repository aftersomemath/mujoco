...
class SignalReport(ReportSection):
    ...
    def __init__(
        ...
        title_prefix: str = "",
        max_datapoints: Optional[int] = 300,
        resample_to_frequency: Optional[float] = None,
        anchor: str = "",
        ts_dict = {},
    ...
    def get_context(self) -> Dict[str, Any]:
        ...
        if self._figure:
            plot_html = self._figure.to_html(full_html=False, include_plotlyjs=False, config=config)
    ...
    def _build_figure(self) -> go.Figure:
        ...
        if n_plots > 1:
            vertical_spacing = min(0.15, 0.5 / (n_plots - 1)) if n_plots > 2 else 0.2
        else:
        ...
        for i, key in enumerate(mapping):
            ...
            for j in range(signal_dim):
                for color_id, ts_name in enumerate(signal_dict):
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=predicted_times, y=predicted_signal[:, j],
                            mode="lines", line=dict(width=2, color=colors[color_id]),
                            opacity=0.8, name=ts_name, legendgroup=ts_name,
    ...
    def _resample_if_needed(self, data: timeseries.TimeSeries) -> timeseries.TimeSeries:
        ...
        if self._resample_to_frequency:
            new_times = np.arange(data.times[0], data.times[-1], 1.0/self._resample_to_frequency)
