...
class ParameterDistribution(ReportSection):
    ...
    def _build_figure(self) -> go.Figure:
        ...
        for param_name, param in self._opt_params.parameters.items():
            ...
            if param.size == 1:
                ...
                if param.frozen:
                    ...
                    conf = self._confidence_intervals[non_frozen_idx] if self._confidence_intervals is not None else None
                    plot_items.append({
                        "name": param_name,
                        "value": self._x_hat[non_frozen_idx],
                        "is_frozen": False,
                        "lb": lb, "ub": ub,
                        "nominal": self._x_nominal[non_frozen_idx] if self._x_nominal is not None else None,
                        "conf": conf
                    })
                    non_frozen_idx += 1
            else:
        ...
        fig.add_trace(go.Scatter(
            ...
            line=dict(color='blue', width=3, dash='dash'), name='Bounds'
        ))
        ...
        for i, item in enumerate(plot_items):
            ...
            if item["is_frozen"]:
                ...
                if item.get("conf") is not None:
                    ...
                    if not np.isfinite(interval) or (rng > 0 and 2.0 * interval > 1.0 * rng):
                         ...
                         if rng > 0: error_val = rng
                         else: error_val = interval # or large value call fallback
                ...
                if show_leg: shown_legends.add(trace_name)
                ...
                if item["nominal"] is not None:
                    ...
                    if show_leg_nom: shown_legends.add("Nominal")
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            legendgroup='nominal',
                            hovertemplate=f"Nominal: %{{y}}<extra></extra>"
                        ),
                        row=row, col=col
                    )
            ...
            fig.update_yaxes(
                ...
                showgrid=True, zeroline=False, row=row, col=col
            )
            fig.update_xaxes(
                range=[-1, 1],
